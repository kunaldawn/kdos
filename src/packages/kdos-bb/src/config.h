...
/*
 * `regparm` is an x86-32 attribute and this target is x86_64, where the SysV
 * ABI already passes six integer arguments in registers. GCC accepts it and
 * warns on every declaration that carries it — 2001's micro-optimisation,
 * now just noise in the build log.
 */
#define REGISTERS(n)
#else
#define CONSTF
