...
{
    /* sizeof(int), not sizeof(long). zbuff is an int array and set_zbuff
     * allocates it as one; on the 32-bit machines this was written for the
     * two were the same size, and on x86_64 the clear ran twice as far as the
     * allocation and corrupted the heap. It aborts in stage 2 with
     * "malloc(): corrupted top size" — found with ASan, not by reading. */
