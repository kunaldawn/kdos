...
static void newline()
{
	...
	/* memmove, not memcpy: this scrolls the buffer up over ITSELF, so
	 * source and destination overlap by everything but one row. glibc
	 * happened to survive it; ASan calls it immediately, and musl is free
	 * to copy in whatever direction it likes. */
