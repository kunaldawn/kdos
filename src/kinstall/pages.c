...
static void sec(int x, int y, int w, const char *title)
{
	draw_text(x, y, w, title, CL_ACCENT, CL_BG, 0);
	int tw = utf8_width(title);
	if (w > tw + 2)
		draw_hline(x + tw + 1, y, w - tw - 1, G_HL, CL_DIM, CL_BG);
}

static void field(int x, int y, int w, const char *k, const char *v, int fg)
{
	draw_text(x, y, 16, k, CL_MID, CL_BG, 0);
	draw_text(x + 17, y, w - 17, v, fg, CL_BG, 0);
}

static void note(int x, int y, int w, const char *s)
{
	draw_text(x, y, w, s, CL_DIM, CL_BG, 0);
}

/* Wrap a paragraph into `w` columns. Returns the number of lines drawn. */
static int para(int x, int y, int w, const char *s, int fg)
{
	int lines = 0;
	while (*s) {
		int take = 0, last = 0;
		...
		if ((int)strlen(s) <= w) {
			draw_text(x, y + lines, w, s, fg, CL_BG, 0);
			return lines + 1;
		}
		if (!last)
			last = take;
		...
		buf[n] = 0;
		draw_text(x, y + lines, w, buf, fg, CL_BG, 0);
		lines++;
		s += last;
		while (*s == ' ')
			s++;
...
static int run_console(char *const argv[])
{
	term_suspend();
	input_suspend();
	pid_t pid = fork();
	...
	if (pid > 0)
		waitpid(pid, &st, 0);
	fputs("\n\033[0m-- press Enter to return to the installer --", stdout);
	fflush(stdout);
	int c;
	while ((c = getchar()) != '\n' && c != EOF)
		;
...
static void welcome_draw(Rect b)
{
	int y = b.y;
	...
	if (b.h >= 22 && b.w > 40 && (term_caps & CAP_UTF8)) {
		for (int i = 0; i < 6; i++)
			draw_text(b.x + (b.w - 33) / 2, y + i, b.w, KDOS_MARK[i],
				  CL_ACCENT, CL_BG, 0);
		y += 7;
	} else {
		draw_text(b.x + (b.w - 14) / 2, y, b.w, "K D O S", CL_ACCENT,
			  CL_BG, 0);
		y += 2;
	}

	const char *tag = "I use KDOS btw.";
	draw_text(b.x + (b.w - (int)strlen(tag)) / 2, y, b.w, tag, CL_MID, CL_BG, 0);
	y += 2;

	sec(b.x, y, b.w, "THIS MACHINE");
	y++;

	char v[128];
	snprintf(v, sizeof(v), "%s  (%d thread%s)", ki_sys.cpu, ki_sys.cores,
		 ki_sys.cores == 1 ? "" : "s");
	field(b.x, y++, b.w, "processor", v, CL_TEXT);
	field(b.x, y++, b.w, "memory",
	      human_size((unsigned long long)ki_sys.mem_kb * 1024), CL_TEXT);
	snprintf(v, sizeof(v), "%d disk%s visible", ki_ndisk,
		 ki_ndisk == 1 ? "" : "s");
	field(b.x, y++, b.w, "storage", v, ki_ndisk ? CL_TEXT : CL_ERR);
	field(b.x, y++, b.w, "install size",
	      human_size((unsigned long long)ki_sys.payload_kb * 1024), CL_TEXT);

	snprintf(v, sizeof(v), "%s%s", ki_sys.uefi ? "UEFI" : "legacy BIOS",
		 ki_sys.secure_boot ? "  (Secure Boot enabled)" : "");
	field(b.x, y++, b.w, "firmware", v, ki_sys.uefi ? CL_TEXT : CL_ERR);
	y++;

	sec(b.x, y, b.w, "PREFLIGHT");
	y++;
	...
	struct {
		...
		{ have_prog("rsync"), "rsync present" },
		{ have_prog("mkfs.ext4") && have_prog("mkfs.vfat"),
		  "mkfs.ext4 and mkfs.vfat present" },
		{ path_exists("/usr/share/refind"), "rEFInd payload present" },
	};

	for (size_t i = 0; i < sizeof(checks) / sizeof(checks[0]); i++) {
		draw_text(b.x, y, 3, checks[i].ok ? ki_glyph[G_SQUARE] : "!",
			  checks[i].ok ? CL_ACCENT : CL_ERR, CL_BG, 0);
		draw_text(b.x + 2, y, b.w - 2, checks[i].msg,
			  checks[i].ok ? CL_TEXT : CL_ERR, CL_BG, 0);
		y++;
	}
		...
		if (dot) {
			...
			for (int i = 0; i < nkeymaps; i++)
				if (!strcmp(keymaps[i], name))
					goto next;
			strlcpy_(keymaps[nkeymaps++], name, sizeof(keymaps[0]));
			continue;
		}
...
static void keymap_load(void)
{
	...
	for (int i = 1; i < nkeymaps; i++) {
		char t[32];
		strlcpy_(t, keymaps[i], sizeof(t));
		int k = i - 1;
		while (k >= 0 && strcmp(keymaps[k], t) > 0) {
			strlcpy_(keymaps[k + 1], keymaps[k], sizeof(t));
			k--;
		}
		strlcpy_(keymaps[k + 1], t, sizeof(t));
	}
	if (!nkeymaps)
		strlcpy_(keymaps[nkeymaps++], "us", sizeof(keymaps[0]));
}
		       ...
		       void *u)
{
	...
	int fg = selected ? CL_BG : cur ? CL_ACCENT : CL_TEXT;
	int bg = selected ? CL_ACCENT : CL_BG;
	draw_text(x + 1, y, 2, cur ? ki_glyph[G_BULLET] : " ", fg, bg, 0);
	draw_text(x + 3, y, w - 3, name, fg, bg, 0);
}
...
static void keyboard_draw(Rect b)
{
	int y = b.y;
	...
	sec(b.x, y, b.w, "CONSOLE KEYMAP");
	y++;
	y += para(b.x, y, b.w,
		  "The layout the text console uses. It is applied immediately "
		  "so you can try it in the box below, and written to "
		  "/etc/keymap on the installed system.", CL_MID);
	y++;

	draw_text(b.x, y, 8, "filter", CL_MID, CL_BG, 0);
	if (ui_input(rect(b.x + 8, y, listw - 8, 1), kmfilter, sizeof(kmfilter),
		     0, "type to narrow"))
		keymap_filter();
	y += 2;
	...
	int lid = ui_id();
	if (ui_list(rect(b.x, y, listw, lh), &kmlist, nkmfilter, keymap_row, NULL,
		    lid)) {
		if (kmlist.sel < nkmfilter) {
			strlcpy_(cfg.keymap, keymaps[kmfilter_idx[kmlist.sel]],
				 sizeof(cfg.keymap));
	...
	if (ui_focused(lid) && kmlist.sel < nkmfilter &&
	...
	if (b.w > listw + 24) {
		...
		int rw = b.x + b.w - rx;
		int ry = y;
		sec(rx, ry, rw, "SELECTED");
		ry += 2;
		draw_text(rx, ry, rw, cfg.keymap, CL_ACCENT, CL_BG, 0);
		ry += 2;
		note(rx, ry++, rw, "try it here:");
		ui_input(rect(rx, ry, rw, 1), kmtest, sizeof(kmtest), 0,
			 "\\ | @ \" # ~");
		ry += 2;
		if (!have_prog("loadkeys"))
			para(rx, ry, rw,
...
static void tz_row(int idx, int x, int y, int w, int selected, int focus, void *u)
{
	...
	int bg = selected ? CL_ACCENT : CL_BG;
	draw_text(x + 1, y, 2, cur ? ki_glyph[G_BULLET] : " ", fg, bg, 0);
	draw_text(x + 3, y, w - 3, zones[z].label, fg, bg, 0);
}
...
static void time_draw(Rect b)
{
	...
	sec(b.x, y, b.w, "TIME ZONE");
	y++;
	y += para(b.x, y, b.w,
		  "KDOS carries no tzdata, so the zone is written as a POSIX TZ "
		  "string that musl reads directly — DST rules included.", CL_MID);
	y++;

	draw_text(b.x, y, 8, "filter", CL_MID, CL_BG, 0);
	if (ui_input(rect(b.x + 8, y, listw - 8, 1), tzfilter, sizeof(tzfilter), 0,
		     "type to narrow"))
		tz_filter();
	y += 2;
	...
	if (lh < 3)
		lh = 3;
	int lid = ui_id();
	ui_list(rect(b.x, y, listw, lh), &tzlist, ntzidx, tz_row, NULL, lid);
	if (tzlist.sel < ntzidx) {
		int z = tzidx[tzlist.sel];
		if (ui_focused(lid)) {
			strlcpy_(cfg.tz_label, zones[z].label, sizeof(cfg.tz_label));
			strlcpy_(cfg.tz, zones[z].tz, sizeof(cfg.tz));
		}
	}
	...
	if (b.w > listw + 24) {
		...
		sec(rx, ry, rw, "SELECTED");
		ry += 2;
		draw_text(rx, ry++, rw, cfg.tz_label, CL_ACCENT, CL_BG, 0);
		ry++;
		field(rx, ry++, rw, "TZ string", cfg.tz, CL_TEXT);
		ry++;
		...
		field(rx, ry++, rw, "local time", when, CL_WARN);
		ry++;
		para(rx, ry, rw,
		     "The hardware clock is left exactly as it is; only the "
		     "environment is set.", CL_DIM);
	}
}
		     ...
		     void *u)
{
	...
	int fg = selected ? CL_BG : warn ? CL_WARN : cur ? CL_ACCENT : CL_TEXT;
	int bg = selected ? CL_ACCENT : CL_BG;

	draw_text(x + 1, y, 2, cur ? ki_glyph[G_BULLET] : " ", fg, bg, 0);
	draw_textf(x + 3, y, w - 3, fg, bg, 0, "%-10s %8s  %-6s %s",
		   d->name,
		   human_size(d->sectors * (unsigned long long)d->sector_size),
	...
	if (!cfg.disk[0]) {
		...
		for (int i = 0; i < ki_ndisk; i++) {
			if (ki_disk[i].is_boot_media || ki_disk[i].readonly)
				continue;
			strlcpy_(cfg.disk, ki_disk[i].path, sizeof(cfg.disk));
			disklist.sel = i;
			break;
		}
	}
...
static void draw_parttable(Rect r, Disk *d)
{
	if (!d->nparts) {
		draw_hline(r.x, r.y, r.w, G_SHADE, CL_DIM, CL_BG);
		draw_text(r.x + 2, r.y, r.w - 4, " unpartitioned ", CL_DIM, CL_BG, 0);
		return;
	}
	for (int i = 0; i < r.w; i++)
		draw_text(r.x + i, r.y, 1, ki_glyph[G_SHADE], CL_DIM, CL_BG, 0);
	...
	for (int i = 0; i < d->nparts; i++) {
		...
		if (x0 + wl > r.w)
			wl = r.w - x0;
		cursor = x0 + wl;
		int c = p->is_esp ? CL_WARN : CL_MID;
		for (int k = 0; k < wl; k++)
			draw_text(r.x + x0 + k, r.y, 1, ki_glyph[G_FULL], c,
				  CL_BG, 0);
		if (wl > 3) {
			char tag[16];
			snprintf(tag, sizeof(tag), "%d", i + 1);
			draw_text(r.x + x0 + 1, r.y, wl - 1, tag, CL_BG, c,
				  A_REVERSE);
		}
	}
}

static void disk_draw(Rect b)
{
	int y = b.y;

	sec(b.x, y, b.w, "TARGET DISK");
	y++;

	int lh = ki_ndisk > 6 ? 6 : (ki_ndisk ? ki_ndisk : 1);
	int lid = ui_id();
	ui_list(rect(b.x, y, b.w, lh), &disklist, ki_ndisk, disk_row, NULL, lid);
	if (ki_ndisk && disklist.sel < ki_ndisk && ui_focused(lid))
		strlcpy_(cfg.disk, ki_disk[disklist.sel].path, sizeof(cfg.disk));
	y += lh + 1;
	...
	sec(b.x, y, b.w, "CURRENT LAYOUT");
	y++;
	draw_parttable(rect(b.x, y, b.w, 1), d);
	y += 2;
	...
	if (d->nparts) {
		draw_textf(b.x, y++, b.w, CL_MID, CL_BG, 0,
			   "  %-12s %-10s %-9s %-9s %s", "PART", "SIZE", "TYPE",
			   "LABEL", "MOUNT");
		for (int i = 0; i < d->nparts; i++) {
			Part *p = &d->part[i];
			draw_textf(b.x, y++, b.w, p->is_esp ? CL_WARN : CL_TEXT,
				   CL_BG, 0, "  %-12s %-10s %-9s %-9s %s",
				   p->name,
				   human_size(p->sectors * 512ULL),
				   p->is_esp ? "ESP"
					     : p->fstype[0] ? p->fstype : "-",
				   p->label[0] ? p->label : "-",
		...
		note(b.x + 2, y++, b.w, "no partition table");
	}
	y++;

	{
		field(b.x, y++, b.w, "capacity", human_size(have), CL_TEXT);
		char v[64];
		snprintf(v, sizeof(v), "%s", human_size(need));
		field(b.x, y++, b.w, "install needs", v,
		      have > need + (512ULL << 20) ? CL_TEXT : CL_ERR);
	}
	...
	if (d->is_boot_media)
		...
		note(b.x, y, b.w, "removable device");
}
...
static int disk_validate(char *err, size_t n)
{
	...
	if (have < need + (768ULL << 20)) {
		snprintf(err, n, "%s holds %s; the install needs at least %s",
			 d->name, human_size(have),
			 human_size(need + (768ULL << 20)));
		return 1;
	}
	return 0;
}
...
static ListState esplist, rootlist;
			     ...
			     int focus, void *u)
{
	...
	Disk *d = (Disk *)u;
	Part *p = &d->part[idx];
	int fg = selected ? CL_BG : CL_TEXT;
	int bg = selected ? CL_ACCENT : CL_BG;
	draw_textf(x + 1, y, w - 1, fg, bg, 0, "%-12s %8s  %-8s %s", p->name,
		   human_size(p->sectors * 512ULL),
...
static void layout_enter(void)
{
	...
	if (!cfg.part_esp[0])
		for (int i = 0; i < d->nparts; i++)
			if (d->part[i].is_esp) {
				strlcpy_(cfg.part_esp, d->part[i].path,
					 sizeof(cfg.part_esp));
				esplist.sel = i;
				break;
			}
...
static void layout_draw(Rect b)
{
	Disk *d = disk_by_path(cfg.disk);
	int y = b.y;

	sec(b.x, y, b.w, "PARTITIONING");
	y++;

	ui_radio(b.x, y++, b.w, "Erase the whole disk and lay it out for KDOS",
		 &cfg.plan, PLAN_WIPE);
	note(b.x + 4, y++, b.w, "GPT: 512M EFI system partition, then root");
	ui_radio(b.x, y++, b.w, "Use partitions that already exist",
		 &cfg.plan, PLAN_REUSE);
	note(b.x + 4, y++, b.w,
	     "keeps every other partition — the dual-boot path");
	ui_radio(b.x, y++, b.w, "Partition it myself first (cfdisk)",
		 &cfg.plan, PLAN_MANUAL);
	y++;

	if (cfg.plan == PLAN_MANUAL) {
		if (ui_button(rect(b.x + 4, y, 26, 1), "Open cfdisk", 1, 0)) {
		...
		para(b.x, y, b.w,
		     ...
		     "installer comes back and switches to picking them.", CL_MID);
		return;
	}
	...
	if (cfg.plan == PLAN_REUSE) {
		...
		sec(b.x, y, half, "EFI SYSTEM PARTITION");
		sec(b.x + half + 3, y, half, "ROOT PARTITION");
		y++;

		int e = ui_id();
		ui_list(rect(b.x, y, half, lh), &esplist, d->nparts,
			part_row_generic, d, e);
		if (ui_focused(e) && esplist.sel < d->nparts)
			strlcpy_(cfg.part_esp, d->part[esplist.sel].path,
				 sizeof(cfg.part_esp));

		int r = ui_id();
		ui_list(rect(b.x + half + 3, y, half, lh), &rootlist, d->nparts,
			part_row_generic, d, r);
		if (ui_focused(r) && rootlist.sel < d->nparts)
			strlcpy_(cfg.part_root, d->part[rootlist.sel].path,
				 sizeof(cfg.part_root));
		y += lh + 1;

		ui_check(b.x, y++, b.w, "Reformat the ESP as FAT32", &cfg.format_esp);
		note(b.x + 4, y++, b.w,
		     "leave this off when another OS boots from the same ESP");
		y++;
		field(b.x, y++, b.w, "ESP", cfg.part_esp[0] ? cfg.part_esp : "-",
		      CL_TEXT);
		field(b.x, y++, b.w, "root",
		      cfg.part_root[0] ? cfg.part_root : "-", CL_ACCENT);
		note(b.x, y++, b.w, "the root partition is reformatted, always");
		return;
	}

	/* PLAN_WIPE */
	sec(b.x, y, b.w, "FILESYSTEM AND SWAP");
	y++;
	field(b.x, y++, b.w, "root filesystem", "ext4", CL_TEXT);
	note(b.x + 17, y++, b.w,
	     "the only mkfs KDOS ships; btrfs and xfs are not ported yet");
	y++;

	ui_radio(b.x, y++, b.w / 2, "No swap", &cfg.swap, SWAP_NONE);
	ui_radio(b.x, y++, b.w / 2, "Swap file on the root filesystem",
		 &cfg.swap, SWAP_FILE);
	ui_radio(b.x, y++, b.w / 2, "Dedicated swap partition", &cfg.swap,
		 SWAP_PART);

	if (cfg.swap != SWAP_NONE) {
		y++;
		draw_text(b.x, y, 16, "size (MiB)", CL_MID, CL_BG, 0);
		if (ui_input(rect(b.x + 17, y, 12, 1), swapbuf, sizeof(swapbuf), 0,
			     "4096")) {
		...
		snprintf(hint, sizeof(hint), "RAM is %s",
			 human_size((unsigned long long)ki_sys.mem_kb * 1024));
		note(b.x + 31, y, b.w, hint);
		y += 2;
	} else {
		y++;
	}
	...
	sec(b.x, y, b.w, "WHAT WILL BE WRITTEN");
	y++;
	...
	for (int i = 0; i < wesp; i++)
		draw_text(bx + i, y, 1, ki_glyph[G_FULL], CL_WARN, CL_BG, 0);
	for (int i = 0; i < wswp; i++)
		draw_text(bx + wesp + i, y, 1, ki_glyph[G_FULL], CL_MID, CL_BG, 0);
	for (int i = 0; i < wroot; i++)
		draw_text(bx + wesp + wswp + i, y, 1, ki_glyph[G_FULL], CL_ACCENT,
			  CL_BG, 0);
	y += 2;
	...
	draw_textf(b.x, y++, b.w, CL_WARN, CL_BG, 0, "  %-14s %-9s %s", nm,
		   human_size(esp), "EFI System, FAT32, mounted at /boot/efi");
	if (swp) {
		pname(d->path, n++, nm, sizeof(nm));
		draw_textf(b.x, y++, b.w, CL_MID, CL_BG, 0, "  %-14s %-9s %s", nm,
			   human_size(swp), "Linux swap");
	}
	pname(d->path, n++, nm, sizeof(nm));
	draw_textf(b.x, y++, b.w, CL_ACCENT, CL_BG, 0, "  %-14s %-9s %s", nm,
		   human_size(root), "ext4, mounted at /");
	y++;

	para(b.x, y, b.w,
	     "Everything currently on this disk is destroyed. Nothing happens "
	     "until you confirm on the summary page.", CL_ERR);
}
...
static int pw_score(const char *p)
{
...
static void pw_meter(int x, int y, int w, const char *p)
{
	...
	int s = pw_score(p);
	int fg = s <= 0 ? CL_ERR : s <= 1 ? CL_WARN : CL_ACCENT;
	for (int i = 0; i < 12; i++)
		draw_text(x + i, y, 1,
			  i < (s + 1) * 3 ? ki_glyph[G_FULL] : ki_glyph[G_SHADE],
			  i < (s + 1) * 3 ? fg : CL_DIM, CL_BG, 0);
	draw_text(x + 14, y, w - 14, names[s], fg, CL_BG, 0);
}
...
static void accounts_draw(Rect b)
{
	int y = b.y;
	int fw = b.w > 44 ? 34 : b.w - 18;

	sec(b.x, y, b.w, "MACHINE");
	y++;
	draw_text(b.x, y, 16, "hostname", CL_MID, CL_BG, 0);
	ui_input(rect(b.x + 17, y, fw, 1), cfg.hostname, sizeof(cfg.hostname), 0,
		 "kdos");
	if (!host_ok(cfg.hostname))
		draw_text(b.x + 18 + fw, y, b.w, "letters, digits and -", CL_ERR,
			  CL_BG, 0);
	y += 2;

	sec(b.x, y, b.w, "YOU");
	y++;
	draw_text(b.x, y, 16, "full name", CL_MID, CL_BG, 0);
	ui_input(rect(b.x + 17, y, fw, 1), cfg.fullname, sizeof(cfg.fullname), 0,
		 "KDOS User");
	y++;
	draw_text(b.x, y, 16, "username", CL_MID, CL_BG, 0);
	ui_input(rect(b.x + 17, y, fw, 1), cfg.username, sizeof(cfg.username), 0,
		 "kdos");
	if (!name_ok(cfg.username))
		draw_text(b.x + 18 + fw, y, b.w, "lowercase, no leading digit",
			  CL_ERR, CL_BG, 0);
	y++;
	draw_text(b.x, y, 16, "password", CL_MID, CL_BG, 0);
	ui_input(rect(b.x + 17, y, fw, 1), cfg.userpass, sizeof(cfg.userpass), 1,
		 "required");
	y++;
	draw_text(b.x, y, 16, "confirm", CL_MID, CL_BG, 0);
	ui_input(rect(b.x + 17, y, fw, 1), cfg.userpass2, sizeof(cfg.userpass2), 1,
		 "");
	if (cfg.userpass2[0] && strcmp(cfg.userpass, cfg.userpass2))
		draw_text(b.x + 18 + fw, y, b.w, "does not match", CL_ERR, CL_BG,
			  0);
	y++;
	pw_meter(b.x + 17, y, b.w - 17, cfg.userpass);
	y += 2;

	ui_check(b.x, y++, b.w, "Administrator (member of wheel, may use sudo)",
		 &cfg.user_wheel);
	y++;

	sec(b.x, y, b.w, "ROOT");
	y++;
	ui_check(b.x, y++, b.w, "Lock the root account (log in as yourself, use sudo)",
		 &cfg.root_locked);

	if (!cfg.root_locked) {
		y++;
		draw_text(b.x, y, 16, "root password", CL_MID, CL_BG, 0);
		ui_input(rect(b.x + 17, y, fw, 1), cfg.rootpass,
			 sizeof(cfg.rootpass), 1, "required");
		y++;
		draw_text(b.x, y, 16, "confirm", CL_MID, CL_BG, 0);
		ui_input(rect(b.x + 17, y, fw, 1), cfg.rootpass2,
			 sizeof(cfg.rootpass2), 1, "");
		if (cfg.rootpass2[0] && strcmp(cfg.rootpass, cfg.rootpass2))
			draw_text(b.x + 18 + fw, y, b.w, "does not match", CL_ERR,
				  CL_BG, 0);
		y += 2;
	} else {
		y++;
	...
	para(b.x, y, b.w,
		     ...
		     "autologin follows the rename.", CL_DIM);
}
...
static void system_draw(Rect b)
{
	int y = b.y;

	sec(b.x, y, b.w, "ACCENT");
	y++;
	note(b.x, y++, b.w,
	...
	for (int i = 0; i < ki_ntheme; i++) {
		int themed = !strcmp(cfg.theme, ki_themes[i].name);
		int v = themed;
		int x = b.x + (i % 4) * col;
		int yy = y + (i / 4) * 2;
		if (ui_radio(x, yy, col - 2, ki_themes[i].label, &v, 1)) {
			strlcpy_(cfg.theme, ki_themes[i].name, sizeof(cfg.theme));
		...
		for (int k = 0; k < 6; k++)
			draw_text(x + 4 + k, yy + 1, 1, ki_glyph[G_FULL],
				  k < 2 ? CL_ACCENT : k < 4 ? CL_MID : CL_WARN,
				  CL_BG, 0);
	}
	y += ((ki_ntheme + 3) / 4) * 2 + 1;

	sec(b.x, y, b.w, "ALIEN APPS");
	y++;
	char lbl[128];
	snprintf(lbl, sizeof(lbl), "Install the alien app library (%s)",
		 human_size((unsigned long long)ki_sys.appbox_kb * 1024));
	ui_check(b.x, y++, b.w, lbl, &cfg.with_appbox);
	y += para(b.x + 4, y, b.w - 4,
		  "The pre-baked Debian container behind every GUI app — "
		  "browser, office, GIMP, the lot. Leaving it out installs a "
		  "much smaller system that can still fetch it later.", CL_DIM);
	y++;

	sec(b.x, y, b.w, "SERVICES AT BOOT");
	y++;
...
static void summary_draw(Rect b)
{
	int y = b.y;
	Disk *d = disk_by_path(cfg.disk);
	char v[192];

	sec(b.x, y, b.w, "REVIEW");
	y++;

	field(b.x, y++, b.w, "keymap", cfg.keymap, CL_TEXT);
	snprintf(v, sizeof(v), "%s  (%s)", cfg.tz_label, cfg.tz);
	field(b.x, y++, b.w, "time zone", v, CL_TEXT);
	field(b.x, y++, b.w, "hostname", cfg.hostname, CL_TEXT);
	snprintf(v, sizeof(v), "%s (%s)%s", cfg.username, cfg.fullname,
		 cfg.user_wheel ? ", administrator" : "");
	field(b.x, y++, b.w, "user", v, CL_TEXT);
	field(b.x, y++, b.w, "root account",
	      cfg.root_locked ? "locked" : "password set",
	      cfg.root_locked ? CL_TEXT : CL_WARN);
	field(b.x, y++, b.w, "accent", cfg.theme, CL_TEXT);
	field(b.x, y++, b.w, "alien apps",
	      cfg.with_appbox ? "installed" : "left out",
	      cfg.with_appbox ? CL_TEXT : CL_WARN);
	...
	field(b.x, y++, b.w, "services", svc[0] ? svc : "none", CL_TEXT);
	y++;
	...
	if (d) {
		snprintf(v, sizeof(v), "%s  %s  %s", d->path,
			 human_size(d->sectors * (unsigned long long)d->sector_size),
			 d->model);
		field(b.x, y++, b.w, "target", v, CL_TEXT);
	}
	field(b.x, y++, b.w, "mode",
	      cfg.plan == PLAN_WIPE ? "erase the disk and repartition"
				    : "use existing partitions",
	      cfg.plan == PLAN_WIPE ? CL_ERR : CL_TEXT);
	if (cfg.plan == PLAN_REUSE) {
		field(b.x, y++, b.w, "ESP", cfg.part_esp, CL_TEXT);
		field(b.x, y++, b.w, "root (reformatted)", cfg.part_root, CL_ERR);
		field(b.x, y++, b.w, "format ESP", cfg.format_esp ? "yes" : "no",
		      CL_TEXT);
	}
	...
	if (cfg.swap != SWAP_NONE)
		snprintf(v + strlen(v), sizeof(v) - strlen(v), ", %ld MiB",
			 cfg.swap_mb);
	field(b.x, y++, b.w, "swap", v, CL_TEXT);
	y++;

	draw_hline(b.x, y, b.w, G_DHL, CL_ERR, CL_BG);
	const char *warn = " EVERYTHING ABOVE THIS LINE IS STILL REVERSIBLE ";
	draw_text(b.x + (b.w - (int)strlen(warn)) / 2, y, b.w, warn, CL_BG,
		  CL_ERR, A_REVERSE);
	y += 2;
	...
	if (cfg.plan == PLAN_WIPE && d)
		y += para(b.x, y, b.w - 2,
			  ...
			  "no confirmation after this page.", CL_ERR);
	else
		y += para(b.x, y, b.w - 2,
			  "Starting the install reformats the root partition. "
			  "Other partitions on the disk are left alone.", CL_WARN);
	y++;
	...
	if (ui_button(rect(b.x, y, 30, 1),
		      cfg.dry_run ? "REHEARSE INSTALL" : "BEGIN INSTALL", 1, 1)) {
		install_plan();
		install_start(0);
		page_goto(8);
	}
	if (ui_button(rect(b.x + 33, y, 24, 1), "Save answer file", 1, 0)) {
		if (conf_save("/tmp/kinstall.conf") == 0)
			...
			modal_alert("Failed", "Could not write /tmp/kinstall.conf");
	}
}
...
static void install_draw(Rect b)
{
	int y = b.y;
	...
	if (log_full) {
		...
		for (int i = 0; i < h && top + i < total; i++)
			draw_text(b.x, y + i, b.w,
				  inst.log[(base + top + i) % LOG_LINES], CL_MID,
				  CL_BG, 0);
		draw_text(b.x, b.y + b.h - 1, b.w,
			  "L back to progress   PgUp/PgDn scroll", CL_DIM, CL_BG,
			  0);
		return;
	}

	sec(b.x, y, b.w, "PROGRESS");
	y++;
	...
	ui_progress(rect(b.x, y, b.w, 1), overall, pct);
	y += 2;

	for (int i = 0; i < inst.nsteps; i++) {
		StepUi *s = &inst.step[i];
		const char *mark = ki_glyph[G_DOT];
		int fg = CL_DIM;
		switch (s->state) {
		case ST_RUNNING:
			mark = ki_glyph[G_RIGHT];
			fg = CL_ACCENT;
			break;
		case ST_DONE:
			mark = ki_glyph[G_SQUARE];
			fg = CL_MID;
			break;
		case ST_FAIL:
			mark = "!";
		...
		draw_text(b.x + 1, y, 2, mark, fg, CL_BG, 0);
		draw_text(b.x + 4, y, 16, s->title,
			  s->state == ST_RUNNING ? CL_TEXT : fg, CL_BG, 0);

		if (s->state == ST_RUNNING) {
			draw_text(b.x + 21, y, b.w - 21,
				  s->note[0] ? s->note : s->detail, CL_MID, CL_BG,
				  0);
			if (s->frac >= 0 && b.w > 60) {
				char p[12];
				snprintf(p, sizeof(p), "%3d%%",
					 (int)(s->frac * 100));
				draw_text(b.x + b.w - 5, y, 5, p, CL_ACCENT,
					  CL_BG, 0);
			}
		} else if (s->state == ST_DONE) {
			char t[24];
			snprintf(t, sizeof(t), "%.1fs", s->t1 - s->t0);
			draw_text(b.x + 21, y, b.w - 21, t, CL_DIM, CL_BG, 0);
		} else if (s->state == ST_SKIP) {
			draw_text(b.x + 21, y, b.w - 21, "not needed", CL_DIM,
				  CL_BG, 0);
		} else if (s->state == ST_FAIL) {
			draw_text(b.x + 21, y, b.w - 21, inst.failmsg, CL_ERR,
				  CL_BG, 0);
		}
		y++;
	}
	...
	int elapsed = (int)(now_s() - inst.t0);
	draw_textf(b.x, y++, b.w, CL_DIM, CL_BG, 0, "elapsed %02d:%02d",
		   elapsed / 60, elapsed % 60);
	y++;

	int tail = b.y + b.h - y - 3;
	if (tail > 2) {
		sec(b.x, y, b.w, "OUTPUT");
		y++;
		int total = inst.nlog < LOG_LINES ? inst.nlog : LOG_LINES;
		int base = inst.nlog > LOG_LINES ? inst.nlog - LOG_LINES : 0;
		...
		for (int i = 0; start + i < total && i < tail; i++)
			draw_text(b.x, y + i, b.w,
				  inst.log[(base + start + i) % LOG_LINES], CL_MID,
				  CL_BG, 0);
		y += tail;
	}
	...
	if (inst.failed) {
		int by = b.y + b.h - 1;
		if (ui_button(rect(b.x, by, 16, 1), "Retry step", 1, 1))
			install_start(inst.cur < 0 ? 0 : inst.cur);
		if (ui_button(rect(b.x + 18, by, 14, 1), "View log", 1, 0))
			log_full = 1;
		if (ui_button(rect(b.x + 34, by, 14, 1), "Shell", 1, 0)) {
			char *a[] = { "/bin/bash", "-l", NULL };
			run_console(a);
		}
		if (ui_button(rect(b.x + 50, by, 14, 1), "Abort", 1, 0))
			ki_quit = 1;
	} else if (inst.done) {
		if (ui_button(rect(b.x, b.y + b.h - 1, 20, 1), "Continue", 1, 1))
			page_goto(9);
	}
}

static int install_event(Event *ev)
{
	if (ev->type == EVT_KEY && (ev->key == 'l' || ev->key == 'L')) {
		log_full = !log_full;
		log_scroll = 0;
		return 1;
	}
	if (log_full && ev->type == EVT_KEY) {
		if (ev->key == K_PGUP) {
			log_scroll += 10;
			return 1;
		}
...
static void done_draw(Rect b)
{
	int y = b.y;

	if (b.h >= 20 && b.w > 40 && (term_caps & CAP_UTF8)) {
		for (int i = 0; i < 6; i++)
			draw_text(b.x + (b.w - 33) / 2, y + i, b.w, KDOS_MARK[i],
				  CL_ACCENT, CL_BG, 0);
		y += 7;
	...
	draw_text(b.x + (b.w - (int)strlen(msg)) / 2, y, b.w, msg, CL_ACCENT,
		  CL_BG, 0);
	y += 2;
	...
	field(b.x, y++, b.w, "took", v, CL_TEXT);
	field(b.x, y++, b.w, "installed to", cfg.disk, CL_TEXT);
	field(b.x, y++, b.w, "log", "/var/log/kinstall.log", CL_TEXT);
	y += 2;

	y += para(b.x, y, b.w,
		  "Remove the boot medium before restarting, or the firmware "
		  "will hand control back to the live image.", CL_MID);
	y++;

	if (ui_button(rect(b.x, y, 20, 1), "Reboot now", 1, 1))
		modal_confirm("Reboot", "Restart this machine now?", "Reboot",
			      "Wait", reboot_now);
	if (ui_button(rect(b.x + 22, y, 20, 1), "Back to the live desktop", 1, 0))
		ki_quit = 1;
	if (ui_button(rect(b.x + 44, y, 14, 1), "View log", 1, 0)) {
