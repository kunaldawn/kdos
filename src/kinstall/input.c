 ...
 * ╚═╝  ╚═╝╚═════╝  ╚═════╝ ╚══════╝
 * ---------------------------------
 *   KDOS Installer — keys and pointers
 *
...
{
	char buf[64];
	if (read_line_file("/sys/class/graphics/fb0/virtual_size", buf,
			   sizeof(buf)) > 0) {
		int px = 0, py = 0;
...
static int evdev_read(Event *ev)
{
	...
	for (int i = 0; i < nmice; i++) {
		while (read(mice[i].fd, &ie, sizeof(ie)) == (ssize_t)sizeof(ie)) {
			if (ie.type == EV_REL) {
				...
				ev->type = EVT_MOUSE;
				ev->btn = b;
				ev->press = ie.value ? MP_PRESS : MP_RELEASE;
				ev->mx = (int)ptr_fx;
				ev->my = (int)ptr_fy;
				return 1;
			}
...
static int decode(Event *ev)
{
	...
	if (c != 0x1b) {
		...
		ev->type = EVT_KEY;
		ev->mods = 0;
		if (c < 0x20 && c != K_ENTER && c != K_TAB && c != 10) {
			ev->key = c + 'a' - 1;
			ev->mods = MOD_CTRL;
			ibuf_drop(1);
			return 1;
	...
	if (ibuf[1] == 'O') {
		...
		ev->mods = 0;
		switch (ibuf[2]) {
		case 'A': ev->key = K_UP; break;
		case 'B': ev->key = K_DOWN; break;
		case 'C': ev->key = K_RIGHT; break;
		case 'D': ev->key = K_LEFT; break;
		case 'H': ev->key = K_HOME; break;
		case 'F': ev->key = K_END; break;
		case 'P': ev->key = K_F1; break;
		case 'Q': ev->key = K_F2; break;
		case 'R': ev->key = K_F3; break;
		case 'S': ev->key = K_F4; break;
	...
	if (sgr_mouse && (fin == 'M' || fin == 'm')) {
		...
		int code = b & 3;
		if (b & 64) {
			ev->btn = code == 0 ? MB_WHEEL_UP : MB_WHEEL_DOWN;
			ev->press = MP_PRESS;
		} else {
			ev->btn = code == 0 ? MB_LEFT : code == 1 ? MB_MIDDLE
								  : MB_RIGHT;
			ev->press = fin == 'm' ? MP_RELEASE
					       : motion ? MP_DRAG : MP_PRESS;
			if (motion && code == 3) {
				ev->btn = MB_MOVE;
				ev->press = MP_DRAG;
			}
	...
	case 'H': ev->key = K_HOME; break;
	case 'F': ev->key = K_END; break;
	case 'Z': ev->key = K_BTAB; break;
	case '~':
		switch (par[0]) {
		case 1: ev->key = K_HOME; break;
		case 2: ev->key = K_INS; break;
		case 3: ev->key = K_DEL; break;
		case 4: ev->key = K_END; break;
		case 5: ev->key = K_PGUP; break;
		case 6: ev->key = K_PGDN; break;
		case 11: ev->key = K_F1; break;
		case 12: ev->key = K_F2; break;
		case 13: ev->key = K_F3; break;
		case 14: ev->key = K_F4; break;
		case 15: ev->key = K_F5; break;
		case 17: ev->key = K_F6; break;
		case 18: ev->key = K_F7; break;
		case 19: ev->key = K_F8; break;
		case 20: ev->key = K_F9; break;
		case 21: ev->key = K_F10; break;
		case 23: ev->key = K_F11; break;
		case 24: ev->key = K_F12; break;
