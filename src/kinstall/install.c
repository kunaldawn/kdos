...
{
	if (!inst.log)
		inst.log = xcalloc(LOG_LINES, LOG_COLS);
	strlcpy_(inst.log[inst.nlog % LOG_LINES], line, LOG_COLS);
	inst.nlog++;
	if (inst.logfd >= 0) {
		char stamp[32];
		time_t t = time(NULL);
...
void install_start(int from_step)
{
	...
	if (pipe(fds) < 0) {
		inst.failed = 1;
		strlcpy_(inst.failmsg, "pipe() failed", sizeof(inst.failmsg));
		return;
	}
	...
	if (pid < 0) {
		...
		close(fds[1]);
		inst.failed = 1;
		strlcpy_(inst.failmsg, "fork() failed", sizeof(inst.failmsg));
		return;
	}
	if (pid == 0) {
		close(fds[0]);
...
static void handle_line(char *s)
{
	...
	case 'N':
		if (inst.cur >= 0)
			strlcpy_(inst.step[inst.cur].note, arg,
	...
	case 'F':
		inst.failed = 1;
		strlcpy_(inst.failmsg, arg, sizeof(inst.failmsg));
		if (inst.cur >= 0) {
			inst.step[inst.cur].state = ST_FAIL;
			inst.step[inst.cur].t1 = now_s();
		}
		install_log(arg);
		break;
	case 'D':
...
void install_abort(void)
{
	...
	inst.running = 0;
	inst.failed = 1;
	strlcpy_(inst.failmsg, "aborted by user", sizeof(inst.failmsg));
}
	...
	for (int pass = 0; pass < 4; pass++) {
		...
		for (char *l = strtok_r(buf, "\n", &save); l && nv < 16;
			...
			if (!strncmp(mnt, prefix, strlen(prefix)))
				strlcpy_(victims[nv++], mnt, sizeof(victims[0]));
		}
		if (!nv)
			return;
			     ...
			     const char *roothash)
{
	...
	if (slurp(path, buf, sizeof(buf)) > 0) {
		...
		if (!cfg.dry_run && write_file(path, out) < 0)
	...
	if (slurp(path, buf, sizeof(buf)) > 0) {
		...
		for (char *l = strtok_r(buf, "\n", &save); l;
			...
			if (!strcmp(name, oldu))
				strlcpy_(name, newu, sizeof(name));
...
static void resolve_parts(void)
{
	if (cfg.plan == PLAN_WIPE) {
		...
		strlcpy_(part_esp, cfg.part_esp, sizeof(part_esp));
		strlcpy_(part_root, cfg.part_root, sizeof(part_root));
	}
...
static void do_format(void)
{
	...
	if (!cfg.dry_run && !path_exists(part_root))
...
static void do_config(void)
{
	...
	strlcpy_(root_uuid, p.uuid, sizeof(root_uuid));
	if (part_esp[0]) {
		probe_part(part_esp, &p);
		strlcpy_(esp_uuid, p.uuid, sizeof(esp_uuid));
	}
	if (part_swap[0]) {
		probe_part(part_swap, &p);
		strlcpy_(swap_uuid, p.uuid, sizeof(swap_uuid));
	}
	...
	wr("/etc/profile.d/20-timezone.sh",
	   ...
	   "export TZ='%s'\n", cfg.tz);
	if (path_exists("/usr/share/zoneinfo") && cfg.tz_label[0]) {
		char zi[256];
		snprintf(zi, sizeof(zi), "/usr/share/zoneinfo/%s", cfg.tz_label);
		if (path_exists(zi) && !cfg.dry_run) {
	...
	if (path_exists("/etc/resolv.conf") && !cfg.dry_run) {
		char rc[4096];
...
static void do_accounts(void)
{
	...
	if (cfg.userpass[0])
		uhash = xstrdup(hash_password(cfg.userpass));
	if (!cfg.root_locked && cfg.rootpass[0])
		rhash = xstrdup(hash_password(cfg.rootpass));
	else
		rhash = "!";
	...
	if (strcmp(cfg.username, "kdos")) {
		...
		if (!cfg.dry_run && path_exists(from) && rename(from, to) < 0)
...
static void do_boot(void)
{
	...
	if (path_exists("/usr/share/kdos/boot/kdos-banner.png"))
		copy_file("/usr/share/kdos/boot/kdos-banner.png",
			  TARGET "/boot/efi/EFI/refind/kdos-banner.png");
	const char *icon = "";
	if (path_exists("/usr/share/kdos/boot/os_kdos.png")) {
	...
	if (have_prog("efibootmgr") && ki_sys.uefi) {
		char disk[64];
		strlcpy_(disk, cfg.disk, sizeof(disk));
