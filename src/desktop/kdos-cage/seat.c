 ...
 * machine so a client sees modifiers, and the pointer path finds the surface
 * under the cursor the same way a real motion does.
 *
...
void
seat_embed_key(struct cg_seat *seat, uint32_t keycode, bool pressed)
{
...
void
seat_embed_motion(struct cg_seat *seat, double lx, double ly, uint32_t time_msec)
{
	if (!seat)
		return;
		  ...
		  uint32_t time_msec)
{
	...
	press_cursor_button(seat, NULL, time_msec, button, state,
			    seat->cursor->x, seat->cursor->y);
	wlr_idle_notifier_v1_notify_activity(seat->server->idle, seat->seat);
}

void
seat_embed_axis(struct cg_seat *seat, int32_t steps, uint32_t time_msec)
{
	if (!seat || steps == 0)
		return;

	/*
	 * A wheel step is 15 degrees, which is what every toolkit's "one notch"
	 * means and what libinput reports for a real wheel. Sending the
	 * discrete count without the value would scroll nothing in a client
	 * that reads only the continuous axis.
	 */
	wlr_seat_pointer_notify_axis(seat->seat, time_msec,
				     WL_POINTER_AXIS_VERTICAL_SCROLL,
				     steps * 15.0, steps * 120,
 ...
 * builds from the environment — the same one a real keyboard would be given, so
 * a guest inside a window and a guest on a VT read the same layout.
 */
void
seat_embed_enable(struct cg_seat *seat)
{
	...
	wlr_keyboard_set_repeat_info(&group->wlr_group->keyboard, 25, 600);
	xkb_keymap_unref(keymap);
	xkb_context_unref(context);
