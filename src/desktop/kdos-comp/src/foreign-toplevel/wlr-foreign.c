...
/*
 * KDOS: a view on another workspace is reported MINIMIZED. The pre-fork
 * compositor did, and kdos-shell's workspace-occupancy derivation and
 * window-list dimming are both built on it — ext-workspace-v1 has no
 * "there are windows here" state, so the panel derives occupancy from
 * exactly this. Omnipresent views are on every workspace by definition.
 */
static bool
kdos_effective_minimized(struct view *view)
{
...
void
kdos_wlr_foreign_sync_minimized(struct wlr_foreign_toplevel *wlr_toplevel)
{
	if (wlr_toplevel->handle) { /* KDOS */
		wlr_foreign_toplevel_handle_v1_set_minimized(
			wlr_toplevel->handle,
			kdos_effective_minimized(wlr_toplevel->view));
	}
}
...
handle_minimized(struct wl_listener *listener, void *data)
{
	...
	wlr_foreign_toplevel_handle_v1_set_minimized(wlr_toplevel->handle,
		kdos_effective_minimized(wlr_toplevel->view)); /* KDOS */
}
