...
/*
 * KDOS: called after a workspace switch or a view moving workspace —
 * every view's wlr handle re-reports minimized so a taskbar sees a
 * window on another workspace as minimized (the pre-fork semantic
 * kdos-shell's occupancy derivation depends on). The ext handle
 * carries identity only and has no state to sync.
 */
void
kdos_foreign_workspace_sync(void)
{
	struct view *view;
	wl_list_for_each(view, &server.views, link) {
		if (view->foreign_toplevel) {
			kdos_wlr_foreign_sync_minimized(
				&view->foreign_toplevel->wlr_toplevel);
		}
	}
}
