...
|---|---|---|
| The ports tree | Stable | 969 recipes. Every one parses, declares its metadata, carries a syntactically valid build script and checksums its sources — all checked by `testing/preflight.sh` |
...
|---|---|---|
| Port recipes | 969 | `find ports/core src/packages src/desktop -name kpkgbuild \| wc -l` |
