...
| **kdos-cage** | **Experimental** | A hard fork of cage 0.3.1 on the wlroots this tree already pins. It builds through its own recipe, links wlroots and XWayland and none of ours dynamically, and the self-test compiles every file in the port wherever wlroots exists. `--embed` is exercised for real by a second process: a guest renders into a shared mapping and an injected keycode changes a later frame. It has also been driven by `kdos-con` end to end — a guest's frames become sprites in the cell grid, a click reaches it at the pixel inside the cell, and snapping, workspaces and closing behave as for any window. One cage speaks for as many toplevels as its guest maps, each with its own headless output and mapping, and each becoming a KDOS window. **No guest has run on a VT**, and **no multi-window guest has been photographed** — that path stands on the compile gate and host runs against a stand-in cage |
...
| **The portal backend** | Stable | Every boxed application's file dialog goes through it, and it opens over the window that asked: `parent_window` is imported through `xdg-foreign` and the compositor centres the dialog on its parent. A guest under `kdos-cage` exports from the cage's own compositor, so that handle resolves to nothing and the dialog is centred on the screen |
...
| **The window model** | In progress | `libkwm` reproduces a 127-row contract taken by reading the compositor line by line, replayed by the self-test and clean under both sanitizers. `kdos-comp` calls five of its entries — `kwm_place`, `kwm_tile_geom`, `kwm_tile_next`, `kwm_ws_adjacent` and, of the edge API, only `kwm_edge_check`. `kdos-con` calls five, including `kwm_fit` for every dragged and snapped rectangle. **No rig pass on the compositor** — that a window lands where a person expects is shown on the console desktop and not yet under Wayland |
| **The display interface** | In progress | `libkdisp` is the one place a surface picks a display server. The conversion moved 33 `kdisp_init` call sites and 174 other calls across `kdos-shell`, `kdos-res` and `kdos-lock`, and every committed reference frame came back byte-identical afterwards. Two implementations exist and every shipped surface registers both — `kcon_impl` and `kwl_impl` — so the seam is proved by use: the same binary is a window on the compositor and a window on the console, with no branch of its own |
| **The console desktop** | **Experimental** | `kdos-con` composites windows, terminals and native surfaces on a character grid, checked against three committed goldens — one offscreen, one two-window, and one taken through a real session and a real view in two processes. Placement and tiling are `libkwm`'s, so they are the compositor's. A graphical application is windows: one `kdos-cage --embed` composites every toplevel its guest maps, each one a window on the grid with its own rectangle, title and place in the stack, the session cuts the frames into sprites, and a view with no pixels prints them as characters — run end to end against a real single-window guest, with clicks, keys, snapping, workspaces and closing all observed; a multi-window guest has been driven only against a stand-in cage on a host. Keys reach a guest as evdev codes with their own releases and the person's own layout, and the pointer in pixels with a two-axis wheel and a lock. **The clipboard crosses the cage boundary in both directions** — a guest's copy arrives as `KEMBED_CLIP_OFFER` and becomes the session's, and the session's clipboard and primary are mirrored back out as `KEMBED_CLIP_SET` onto every cage's own seat, each over a sealed `memfd`; text only, bounded at 64 KB. Proved on a host with stand-in cages in both directions — one guest's copy reaching a second guest, and a copy made in the session reaching both — and **never exercised against a real toolkit**, which is where the mime negotiation is. **A drag crosses it too, in both directions** — the session sends `KEMBED_DRAG_ENTER` and its three over the channel and the cage replays them as a real `wlr_drag` driving a pointer grab, so the guest's own refusal of a type still decides the drop; a guest's own drag is read out as `KEMBED_DRAG_OFFER` at the moment it starts, so the session is carrying it before the pointer leaves the window. Compile-gated against wlroots 0.20's headers and **never dragged with a pointer**, which is the same untested half as the clipboard's. Booted, and driven. The ISO reaches a cell desktop on `tty1` with nothing typed; the clock advances across a 60-second photograph, so the grid is redrawn rather than frozen. Verified on the running system: `Super+Return` opens a framed window that appears in the taskbar and shows its child's output; clicking `Start` opens the menu and a pager cell switches workspace; `Shift+PageUp` reaches the scrollback; a toast and the desktop icon layer draw as layers, unframed and out of the taskbar; a graphical application takes a terminal of its own and returns; `kdos con kill` ends a named session; and **detach and re-attach lose nothing** — the view exits, the session keeps running, and re-attaching restores every window with the text that was typed before the detach. The pointer takes a shape from what is under it — resize arrows per edge and corner, a move handle on a title row, an I-beam over a terminal — decided by the session and drawn only by a view with pixels; the mapping is asserted at ten places on one window and **no shape has been photographed**, because the rig's display does not run that backend |
| **kdos-view** | **Experimental** | Compiles and **links** against real drm, input, seat, xkb, udev, fcft and pixman. `--kms` sets a mode on the ISO — 1280x800 on a virtio connector, seat active, a 7x13 cell and a 182x61 grid, reported on one line at start-up — and reads the keyboard, which is what opens a window. `--cast` is proved too: it registers a PipeWire node and a real consumer captured a frame of the console desktop, chrome, taskbar, clock and all. `--dump` and `--tty` turn a picture into characters by shape, which is how an embedded application and an animation are checked |
...
| **Drag and drop** | **Experimental** | Both directions on both desktops: `libkwl` has a data source on the send side and an accepted offer on the receive side, and the console session carries one itself — `DRAG_START` from a surface, then `ENTER`/`MOTION`/`LEAVE`/`DROP` to whatever is under the pointer, with the payload held until the release and the icon layer asked last so a drop can reach the trash. `text/plain` and `text/uri-list`. The four verbs and their order are asserted over a socketpair. **An embedded graphical application is a target too**, over its cage's own channel: the same four verbs go out as `KEMBED_DRAG_*` and the cage turns them into `wl_data_device` events, and a drag a guest begins is read out and taken over by the session. **Never dragged with a pointer**, on either desktop or across the boundary. No rig pass |
