...
**A login on `tty1` reaches it** without anyone typing a command: `kdos-login` autologins the
account `login.conf` names and the shell's profile starts `kdos-desktop`. There is no display
manager and no greeter.
...
desktop supplies the terminal, which is `foot`. The entry
...
every window re-fits against the row it frees, and a docked panel goes with it.
...
**A link opens the same way a file does.** `mailto:` reaches `aerc`; `http`, `https` and a saved
page reach the browser you installed as a box, and `w3m` where no browser pack is installed.
...
A handler that wants a terminal is given one wherever it is launched from.
...
a new lock client can replace it. The unlock is a message the lock client sends, never something
inferred from it exiting.
...
is indistinguishable from a crashed session. Set any `idle_*` key in `~/.config/kdos/comp.conf` to
turn them on anyway. See
