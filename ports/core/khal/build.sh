...
# `pypackages` names the rest of khal's runtime set and `ports/fetch` crawls
# each tarball's own build-system.requires from there, so one bundle carries
# what a dozen ports would — the same shape python3-platformdirs and
# python3-meshtastic use. click, python-dateutil, six and typing-extensions
# come from their python3-* ports: a second copy in site-packages would be a
# path two packages own, and kpkg refuses that as a conflict.
mkdir -p vendor
...
pip3 install --no-deps --no-index --find-links=vendor --no-build-isolation \
	--root=$PKG --prefix=/usr \
	click-log configobj icalendar pytz pyxdg tzdata tzlocal urwid wcwidth .
