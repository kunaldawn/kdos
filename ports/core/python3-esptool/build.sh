...
# click and rich (with markdown-it-py and mdurl) are ports, and a resolving
# install would leave out whichever of the bundle's modules another package had
# already put in the build root. Build isolation stays ON and pointed at the
# bundle, which carries the backends its sdists name.
#
...
BUILD_EXTENSION=yes pip3 install --no-deps --no-index --find-links=vendor --root=$PKG --prefix=/usr \
	bitarray bitstring esp-pylib intelhex reedsolo rich-click websockets .
