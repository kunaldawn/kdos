#!/bin/bash
# ██╗  ██╗██████╗  ██████╗ ███████╗
# ██║ ██╔╝██╔══██╗██╔═══██╗██╔════╝
# █████╔╝ ██║  ██║██║   ██║███████╗
# ██╔═██╗ ██║  ██║██║   ██║╚════██║
# ██║  ██╗██████╔╝╚██████╔╝███████║
# ╚═╝  ╚═╝╚═════╝  ╚═════╝ ╚══════╝
# ---------------------------------
#   KD's Homebrew Linux Distro
# ---------------------------------

# NOT ./autogen.sh: the release tarball ships Makefile.am and no autogen.sh —
# that script is only in the git tree. autoreconf does the same work from what
# is here.
autoreconf -fi

# The four optional libraries are what the parsers need and each is answered by
# silently dropping support rather than by failing: without jansson there is no
# JSON output for an editor to read, without libxml2 no XML/XSLT/Maven parsing,
# without pcre2 no multiline regex parsers, and without yaml no Ansible or
# OpenAPI. A ctags that claims 140 languages and parses 130 is the failure.
# libseccomp is the sandbox behind --_interactive=sandbox, the mode an editor
# drives ctags in. rst2man is named by path because without it the build
# succeeds and ships no man pages; pegof is not a port, so the PEG parsers are
# the ones generated by the bundled packcc.
./configure \
	--prefix=/usr \
	--enable-json \
	--enable-xml \
	--enable-yaml \
	--enable-pcre2 \
	--enable-seccomp \
	--with-rst2man=/usr/bin/rst2man \
	--with-pegof=no
make
make DESTDIR=$PKG install
